...
- No era codigo: la fila de `channel_translations` para `locale = en` tenia el `home_seo` (meta_title/description/keywords) y `name` con los valores demo por defecto de api-platform ("Demo store"), nunca actualizados, mientras que `es` ya tenia "PHPeitor Store".
...
- Se crearon `FRONTEND_ARQUITECTURE.md`, `FRONTEND_STANDARDS.md`, `BACKEND_ARQUITECTURE.md`, `BACKEND_STANDARDS.md`, `DATABASE_STANDARDS.md`, `AGENTS_ROLES.md` para dejar contexto reusable del stack real (api-platform/Laravel 11, Vue 3 sin SFC via `x-template`, Tailwind, pipeline de imagenes, HTMLPurifier, cache de pagina completa, EAV de productos) y protocolo de trabajo para iteraciones futuras con agentes IA.
